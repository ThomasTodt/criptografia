...
void postencode(char* s, char* key, char* result)
{
  ...
  for (int i=0; i < strlen(result); i++) 
  {
    tmp = (rand() % 13) * 2;
    // printf("numero: %d\n", tmp);
    while(('a' + tmp) == key[key_separator] || ('a' + tmp + 1) == key[key_separator])
      tmp = (rand() % 13) * 2;

    if (result[i] == '.')
    {
      result[i] = 'a' + tmp;//novo_alfabeto[tmp+1];
    //   printf("letra: %c\n", novo_alfabeto[tmp-2]);
    }
    else if (result[i] == '-')
    {
      result[i] = 'a' + tmp + 1; //novo_alfabeto[tmp];
    //   printf("letra: %c\n", novo_alfabeto[tmp-1]);
    }
    else if (result[i] == '/')
    {
...
//   int key_separator = (strlen(key) / 2)+1;
...
//       s[i] = '/';
//       key_separator++;
//       if (key_separator == strlen(key)-1) key_separator = (strlen(key) / 2)+1;
//     }
//     else if (s[i] % 2)
//     {
//       s[i] = '.';
